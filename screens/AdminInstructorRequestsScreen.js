...
import { getInstructorRequests, approveInstructor, rejectInstructor } from '../services/api';
...
export default function AdminInstructorRequestsScreen({ navigation }) {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await getInstructorRequests(); setRequests(r.data.instructors); }
    catch { setRequests([]); }
    setLoading(false);
  };
  ...
  const handleApprove = (item) => {
    Alert.alert('Approve', `Approve ${item.fullName}?`, [
      ...
      { text: 'Approve', onPress: async () => {
        setActing(item._id);
        try { await approveInstructor(item._id); load(); }
  ...
  const handleReject = (item) => {
    Alert.alert('Reject', `Reject and remove ${item.fullName}?`, [
      ...
      { text: 'Reject', style: 'destructive', onPress: async () => {
        setActing(item._id);
        try { await rejectInstructor(item._id); load(); }
            ...
            renderItem={({ item }) => (
              <View style={s.card}>
                ...
                <View style={s.details}>
                  ...
                  <Text style={s.detail}>🗓 Registered {formatDate(item.createdAt)}</Text>
                </View>
...
const s = StyleSheet.create({
  ...
  header:      { flexDirection: 'row', alignItems: 'center', paddingTop: 56, paddingHorizontal: 20, paddingBottom: 20, gap: 14 },
  back:        { padding: 4 },
  ...
  pendingBadge:{ backgroundColor: '#FFF3CD', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4 },
  pendingText: { color: '#E07B00', fontSize: 11, fontWeight: '800' },
  details:     { backgroundColor: '#F4F6FF', borderRadius: 10, padding: 12, gap: 5, marginBottom: 14 },
  detail:      { fontSize: 13, color: '#444' },
  actions:     { flexDirection: 'row', gap: 10 },
  approveBtn:  { flex: 1, backgroundColor: '#2EAB6F', borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
  approveTxt:  { color: '#fff', fontWeight: '800', fontSize: 14 },
  rejectBtn:   { flex: 1, backgroundColor: '#FFE5E5', borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
